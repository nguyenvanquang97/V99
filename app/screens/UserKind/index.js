		...
		<View style={tw`flex bg-white min-h-full content-between`}>
					...
					<View style={tw`flex items-center`}>
						...
						{currentUser && currentUser.userKind ?
							<>
								...
								<Text style={tw`text-white font-medium text-lg`}>Chiết
									khấu: {currentUser && currentUser.userKind ? currentUser.userKind.chietkhau : 'Chưa có'}%</Text>
								<Text style={tw`text-white font-medium text-lg`}>Giảm giá mua
									hàng: {currentUser && currentUser.userKind ? currentUser.userKind.giamgia : 'Chưa có'}%</Text>
							</>
							:
							<View style={tw`bg-white opacity-50 px-3 py-2 rounded`}>
								<Text style={tw`text-blue-500 text-xl font-medium`}>Tài khoản thường</Text>
							</View>
						}
			...
			<View style={tw`my-3 mx-3 bg-orange-50 border border-orange-200 flex items-center p-5 rounded`}>
				...
				<TouchableOpacity
					...
					onPress={() => props.navigation.navigate('Modal', {content: <UpgradeAccount navigation={props.navigation} backScreen={'UserKind'} />})}
				>
