				...
				<View style={tw`mb-10 bg-white mt-5`}>
					...
					{transactionInfo.transaction.point > 0 &&
						<View style={tw`flex items-center justify-between flex-row px-3 py-3 border-b border-gray-200`}>
							...
							<Text
								style={tw`font-medium text-base ${transactionInfo.transaction.kind === 'IN' ? 'text-blue-500' : 'text-red-600'}`}>
								{transactionInfo.transaction.kind === 'IN' ? '+' : '-'}{formatVND(transactionInfo.transaction.point)}
							</Text>
						</View>
					}
					{transactionInfo.transaction.remainAmount > 0 &&
						<View style={tw`flex items-center justify-between flex-row px-3 py-3 border-b border-gray-200`}>
							<Text style={tw`font-medium`}>
								Còn lại
							</Text>
							<Text
								...
								{formatVND(transactionInfo.transaction.remainAmount)}
							</Text>
						</View>
					}
					...
					{((transactionInfo.transaction.type === 'Đầu tư') || (transactionInfo.transaction.type === 'Nâng cấp tài khoản') || (transactionInfo.transaction.type === 'Nạp tiền')) && transactionInfo.transaction.status === 'Chờ xác nhận' &&
						<View style={tw`px-3 mt-5`}>
							...
							<View style={tw`mb-5 flex items-center`}>
								<Image source={{uri: `https://img.vietqr.io/image/${settings && settings.bank_code}-${settings && settings.bank_account}-${settings && settings.payment_qr_template}.jpg?amount=${transactionInfo.transaction.amount}&addInfo=V99+${transactionInfo.transaction.id}`}} style={tw`w-32 h-32`} />
							</View>
							<View style={tw`mb-3 border-b border-gray-100 pb-2`}>
								...
								<View style={tw`flex flex-row items-center justify-between mb-2`}>
									...
									<View>
										<Text
											style={tw`font-medium`}>{transactionInfo && formatVND(transactionInfo.transaction.amount)}</Text>
										<TouchableOpacity
											onPress={() => copyToClipboard(transactionInfo.transaction.amount)}
											style={tw`flex flex-row items-center`}
										>
											<Icon name="content-copy" style={tw`text-blue-400 mr-1`} />
											<Text style={tw`text-blue-400`}>Sao chép</Text>
								...
								<View style={tw`flex flex-row items-center justify-between mb-2`}>
									<Text>Nội dung</Text>
									<View>
										<Text style={tw`font-medium`}>V99-{transactionInfo.id}</Text>
										<TouchableOpacity
											onPress={() => copyToClipboard(`V99-${transactionInfo.id}`)}
											style={tw`flex flex-row items-center`}
										>
											<Icon name="content-copy" style={tw`text-blue-400 mr-1`} />
											<Text style={tw`text-blue-400`}>Sao chép</Text>
