...
const HomeScreen = props => {
  ...
  const categories = useSelector(
    state => state.SettingsReducer.productCategories,
  );
  ...
  useEffect(() => {
    let timer1 = setTimeout(
      () => setTextSearchHolder('Tìm kiếm sản phẩm...'),
      3000,
    );
    return () => {
      clearTimeout(timer1);
  ...
  return (
    <View
      style={{
        ...
        ...tw`flex bg-gray-100 min-h-full`,
      }}
    >
      ...
      <View
        style={[
          tw`${
            Platform.OS === 'android' ? 'pt-4' : 'pt-14'
          } pb-2 px-3 bg-white shadow-lg`,
        ]}
      >
        <View style={tw`flex flex-row items-center justify-between`}>
          ...
          <CartIcon navigation={props.navigation} dark />
          <TouchableOpacity
            activeOpacity={1}
      ...
      >
        {loading ? (
          ...
          <View style={tw`pb-20`}>
            <Slideshow
              items={
                slideshows && slideshows.items && JSON.parse(slideshows.items)
              }
              navigation={props.navigation}
            />
            ...
            {!currentUser && (
              <View
                style={tw`mb-5 bg-gray-50 border border-green-600 rounded p-3 mx-3`}
              >
                <Text style={tw`text-gray-800 text-center`}>
                  <Text style={tw`font-medium`}>Đăng nhập</Text> vào tài khoản
                  của bạn để hưởng nhiều ưu đãi hơn khi mua hàng và quản lý đơn
                  hàng dễ dàng hơn!
                </Text>
            ...
            <ShopList
              ...
              type={'Sản phẩm'}
            />
            <News horizontal={true} navigation={props.navigation} />
          </View>
        )}
      </ScrollView>
    </View>
