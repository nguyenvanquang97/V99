					...
					<View style={tw`pb-32`}>
						<View style={tw`mb-3 bg-white p-3`}>
							<View style={tw`flex items-center mb-3`}>
								<Text  style={tw`mb-3 text-gray-500`}>Mã đơn hàng {result.order.type}: <Text style={tw`font-bold text-gray-800`}>#{result.order.id}</Text></Text>
								{result && result.order.orderSort > 0 &&
									<View style={tw`mb-3`}>
										<Text>Thứ tự đơn hàng <Text style={tw`font-bold text-purple-500`}>{result && result.order.orderSort}</Text></Text>
									</View>
								}
								<View style={tw`flex items-center flex-row`}>
									<Text style={tw`font-medium`}>{result && result.order.status}</Text>
									{result && result.order.process &&
										<Text style={tw`ml-2 font-medium`}>{result && result.order.process}</Text>
									}
								</View>
							...
							<View>
								...
								{result && result.order.cash > 0 &&
									<View style={tw`py-2 border-b border-gray-100 flex flex-row justify-between`}>
										<Text>Tiền mặt</Text>
										<Text style={tw`font-medium text-green-600`}>-{formatVND(result.order.cash)}</Text>
									</View>
								}
								{result && result.order.point > 0 &&
									<View style={tw`py-2 border-b border-gray-100 flex flex-row justify-between`}>
										<Text>Ví điểm</Text>
										<Text style={tw`font-medium text-purple-600`}>-{formatVND(result.order.point)}</Text>
									</View>
								}
								{result && result.order.tieudungAmount > 0 &&
									<View style={tw`py-2 border-b border-gray-100 flex flex-row justify-between`}>
										<Text>Ví tiêu dùng</Text>
										<Text style={tw`font-medium text-orange-600`}>-{formatNumber(result.order.tieudungAmount)} BTM</Text>
									</View>
								}
								{result && result.order.paymentMethod && result.order.paymentMethod !== 'Ví tiền' && result.order.paymentMethod !== 'Ví điểm' &&
									<View style={tw`py-2 border-b border-gray-100 flex flex-row justify-between`}>
										...
										<Text style={tw`font-medium text-blue-600`}>{result.order.paymentMethod}</Text>
									</View>
								}
								...
								<View style={tw`mb-5 flex items-center`}>
									<Image source={{uri: `https://qr.sepay.vn/img?acc=${settings && settings.bank_account}&bank=${settings && settings.bank_code}&amount=${result.order.cash}&des=${result.order.paymentNote}`}} style={tw`w-32 h-32`} />
								</View>
