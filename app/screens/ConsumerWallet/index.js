...
function ConsumerWalletScreen(props) {
			...
			<View style={tw`bg-orange-500 px-3 pt-12 pb-3`}>
					...
					<TouchableOpacity
						...
						onPress={() => props.navigation.navigate('Modal', {
							content: <TransferScreen
								navigation={props.navigation}
								backScreen={'ConsumerWallet'}
								wallet={'Ví tiêu dùng'}
							/>
						})}
					>
						<Icon name={"transfer"} size={24} style={tw`text-white mr-1`} />
				...
				<View style={tw`bg-orange-500 pb-2`}>
					<View style={tw`flex items-center`}>
						<Text  style={tw`text-white mb-3 font-medium text-lg`}>Ví tiêu dùng</Text>
						<Text  style={tw`text-white text-xs mb-1`}>Số dư ví (BTM)</Text>
						<Text  style={tw`text-white font-bold text-4xl`}>{currentUser && formatNumber(currentUser.consumerWallet)}</Text>
					</View>
				</View>
			</View>
			<ScrollView
...
export default ConsumerWalletScreen;
