								...
								{result && result.paymentMethod === 'BankTransfer' &&
									<View>
										...
										<View style={tw`mb-5 flex items-center`}>
											<Image source={{uri: `https://img.vietqr.io/image/${settings && settings.pc_bank_code}-${settings && settings.pc_bank_account}-${settings && settings.mk_payment_qr_template}.jpg?amount=${result.amount}&addInfo=HTMI+${result.id}`}} style={tw`w-32 h-32`} />
										</View>
										<View style={tw`mb-3 border-b border-gray-100 pb-2`}>
											...
											<View style={tw`flex flex-row items-center justify-between mb-2`}>
												<Text>Nội dung</Text>
												<View>
													<Text style={tw`font-medium`}>HTMI-{result.id}</Text>
													<TouchableOpacity
														onPress={() => copyToClipboard(`HTMI-${result.id}`)}
														style={tw`flex flex-row items-center`}
													>
														<Icon name="content-copy" style={tw`text-blue-400 mr-1`} />
														<Text style={tw`text-blue-400`}>Sao chép</Text>
