				...
				<View style={tw`flex bg-gray-100 min-h-full content-between`}>
					...
					>
						<View style={tw`pb-52 pt-3 px-5`}>
							...
							>
								<View style={tw`flex flex-row items-center`}>
									<Icon name={paymentMethod === 'PointWallet' ? 'radiobox-marked' : 'radiobox-blank'}
									      size={18} style={tw`mr-1 text-blue-500`} />
									<Text style={tw`font-bold`}>
										Ví điểm thưởng ({currentUser && currentUser && formatVND(currentUser.pointWallet)})
									</Text>
								</View>
								<Text style={tw`italic text-xs`}>
							...
							>
								<View style={tw`flex flex-row items-center`}>
									<Icon name={paymentMethod === 'BankTransfer' ? 'radiobox-marked' : 'radiobox-blank'} size={18} style={tw`mr-1 text-blue-500`} />
									<Text style={tw`font-bold`}>
					...
					<View style={tw`absolute bottom-0 android:bottom-14 bg-white w-full pb-5 pt-1 shadow-lg px-3`}>
						<View style={tw`mb-2`}>
							...
							<View style={tw`flex flex-row items-center justify-between mb-1`}>
								<Text  >Tổng tiền</Text>
								<Text style={tw`text-blue-500 text-base font-bold`}>{formatVND(state.price.subTotal - state.price.discount - state.price.userKindDiscount)}</Text>
							</View>
						</View>
						<TouchableOpacity
